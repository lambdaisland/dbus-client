...
OK Command
   ...
   The OK command has one argument, which is the GUID of the server. See [91]the section called “Server Addresses” for more on server GUIDs.
...
org.freedesktop.DBus.Peer
   ...
   On Windows, the hardware profile GUID is used as the machine ID, with the punctuation removed. This can be obtained with the [112]GetCurrentHwProfile function.
...
Eavesdropping
   ...
   Eavesdropping interacts poorly with buses with non-trivial access control restrictions, and is deprecated. The BecomeMonitor method (see [126]the section called “Method: org.freedesktop.DBus.Monitoring.BecomeMonitor”) provides a preferable way to monitor buses.
...
Match Rules
   ...
   Match rules can also be used for eavesdropping (see [128]the section called “Eavesdropping”), if the security policy of the message bus allows it, but this usage is deprecated in favour of the BecomeMonitor method (see [129]the section called “Method: org.freedesktop.DBus.Monitoring.BecomeMonitor”).

   Match rules are added using the AddMatch bus method (see [130]the section called “Method: org.freedesktop.DBus.AddMatch”). Rules are specified as a string of comma separated key/value pairs. Excluding a key from the rule indicates a wildcard match. For instance excluding the the member from a match rule but adding a sender would let all messages from that sender through. An example of a complete rule would be "type='signal',sender='org.freedesktop.DBus',interface='org.freedesktop.DBus',member='Foo',path='/bar/foo',destination=':452345.34',arg2='bar'"

   Within single quotes (ASCII apostrophe, U+0027), a backslash (U+005C) represents itself, and an apostrophe ends the quoted section. Outside single quotes, \' (backslash, apostrophe) represents an apostrophe, and any backslash not followed by an apostrophe represents itself. For instance, the match rules arg0=''\''',arg1='\',arg2=',',arg3='\\' and arg0=\',arg1=\,arg2=',',arg3=\\ both match messages where the arguments are a 1-character string containing an apostrophe, a 1-character string containing a backslash, a 1-character string containing a comma, and a 2-character string containing two backslashes[131]^[2].
   ...
   sender A bus or unique name (see [132]Bus Name and [133]Unique Connection Name respectively) Match messages sent by a particular sender. An example of a sender match is sender='org.freedesktop.Hal'
   interface An interface name (see [134]the section called “Interface names”) Match messages sent over or to a particular interface. An example of an interface match is interface='org.freedesktop.Hal.Manager'. If a message omits the interface header, it must not match any rule that specifies this key.
   ...
   path An object path (see [135]the section called “Valid Object Paths”) Matches messages which are sent from or to the given object. An example of a path match is path='/org/freedesktop/Hal/Manager'
   ...
   destination A unique name (see [136]Unique Connection Name) Matches messages which are being sent to the given unique name. An example of a destination match is destination=':1.0'
   ...
   See also [137]the section called “Signal: org.freedesktop.DBus.NameOwnerChanged”.

   This match key was added in version 0.16 of the D-Bus specification and implemented by the bus daemon in dbus 1.5.0 and later.
   eavesdrop 'true', 'false'

   Since D-Bus 1.5.6, match rules do not match messages which have a DESTINATION field unless the match rule specifically requests this (see [138]the section called “Eavesdropping”) by specifying eavesdrop='true' in the match rule. eavesdrop='false' restores the default behaviour. Messages are delivered to their DESTINATION regardless of match rules, so this match does not affect normal delivery of unicast messages. In older versions of D-Bus, this match was not allowed in match rules, and all match rules behaved as if eavesdrop='true' had been used.

   Use of eavesdrop='true' is deprecated. Monitors should prefer to use the BecomeMonitor method (see [139]the section called “Method: org.freedesktop.DBus.Monitoring.BecomeMonitor”), which was introduced in version 0.26 of the D-Bus specification and version 1.9.10 of the reference dbus-daemon.
...
Message Bus Starting Services (Activation)
   ...
   It is also possible for applications to send an explicit request to start a service: this is another form of activation, distinct from auto-starting. See [140]the section called “Method: org.freedesktop.DBus.StartServiceByName” for details.
   ...
   To find an executable corresponding to a particular name, the bus daemon looks for service description files. Service description files define a mapping from names to executables. Different kinds of message bus will look for these files in different places, see [141]the section called “Well-known Message Bus Instances”.

   Service description files have the ".service" file extension. The message bus will only load service description files ending with .service; all other files will be ignored. The file format is similar to that of [142]desktop entries. All service description files must be in UTF-8 encoding. To ensure that there will be no name collisions, service files must be namespaced using the same mechanism as messages and service names.
...
systemd Activation
   ...
   Service description files may contain a SystemdService key. Its value is the name of a [144]systemd service, for example dbus-com.example.MyDaemon.service.
...
Mediating Activation with AppArmor
   ...
   Please refer to [145]AppArmor documentation for general information on AppArmor, and how it mediates D-Bus messages when used in conjunction with a kernel and dbus-daemon that support this.
   ...
   Explicit activation via the [146]StartServiceByName method call is not currently affected by this mediation: if a confined process is to be prevented from starting arbitrary services, then it must not be allowed to call that method.
...
Finding session services
   ...
   On Unix systems, the session bus should search for .service files in $XDG_DATA_DIRS/dbus-1/services as defined by the [147]XDG Base Directory Specification. Implementations may also search additional locations, with a higher or lower priority than the XDG directories.
...
Message Bus Interface: org.freedesktop.DBus
   ...
   The special message bus name org.freedesktop.DBus responds to a number of additional messages at the object path /org/freedesktop/DBus, implementing the org.freedesktop.DBus interface. That object path is also used when emitting signals such as [149]NameOwnerChanged.
...
Method: org.freedesktop.DBus.ListQueuedOwners
   ...
   List the connections currently queued for a bus name (see [150]Queued Name Owner).
...
Signal: org.freedesktop.DBus.ActivatableServicesChanged
   ...
   This signal is sent when the list of activatable services, as returned by ListActivatableNames(), might have changed (see [151]the section called “Method: org.freedesktop.DBus.ListActivatableNames”). Clients that have cached information about the activatable services should call ListActivatableNames() again to update their cache.

   The presence of this signal is indicated by a bus feature property (for details see [152]the section called “Property: org.freedesktop.DBus.Features”). In older implementations that do not have this feature, there is no way to be informed when the list of activatable names has changed.
...
Method: org.freedesktop.DBus.StartServiceByName
   ...
   Tries to launch the executable associated with a name (service activation), as an explicit request. This is an alternative to relying on auto-starting. For more information on how services are activated and the difference between auto-starting and explicit activation, see [153]the section called “Message Bus Starting Services (Activation)”.

   It is often preferable to carry out auto-starting instead of calling this method. This is because calling this method is subject to a [154]time-of-check/time-of-use issue: if a caller asks the message bus to start a service so that the same caller can make follow-up method calls to that service, the fact that the message bus was able to start the required service is no guarantee that it will not have crashed or otherwise exited by the time the caller makes those follow-up method calls. As a result, calling this method does not remove the need for the caller to handle errors from method calls. Given that fact, it is usually simpler to rely on auto-starting, in which the required service starts as a side-effect of the first method call.
...
Method: org.freedesktop.DBus.GetConnectionCredentials
   ...
   ProcessFD UNIX_FD A file descriptor pinning the process, on platforms that have this concept. On Linux, the SO_PEERPIDFD socket option is a suitable implementation. This is safer to use to identify a process than the ProcessID, as the latter is subject to re-use attacks, while the FD cannot be recycled. If the original process no longer exists the FD will no longer be resolvable.
   ...
   On Linux systems, the security label that would result from the SO_PEERSEC getsockopt call. The array contains the non-zero bytes of the security label in an unspecified ASCII-compatible encoding[155]^[a], followed by a single zero byte.

   For example, the SELinux context system_u:system_r:init_t:s0 (a string of length 27) would be encoded as 28 bytes ending with ':', 's', '0', '\x00'.[156]^[b]
   ...
   [157]^[a] It could be ASCII or UTF-8, but could also be ISO Latin-1 or any other encoding.

   [158]^[b] Note that this is not the same as the older GetConnectionSELinuxContext method, which does not append the zero byte. Always appending the zero byte allows callers to read the string from the message payload without copying.

   This method was added in D-Bus 1.7 to reduce the round-trips required to list a process's credentials. In older versions, calling this method will fail: applications wishing to support obsolete message bus versions can recover by using the separate methods such as [159]GetConnectionUnixUser instead.
...
Method: org.freedesktop.DBus.GetAdtAuditSessionData
   ...
   Returns auditing data used by Solaris ADT, in an unspecified binary format. If you know what this means, please contribute documentation via the D-Bus bug tracking system. This method is on the core DBus interface for historical reasons; the same information should be made available via the [160]GetConnectionCredentials method in future.
...
Method: org.freedesktop.DBus.GetConnectionSELinuxSecurityContext
   ...
   Returns the security context used by SELinux, in an unspecified format. If you know what this means, please contribute documentation via the D-Bus bug tracking system. This method is on the core DBus interface for historical reasons; the same information should be made available via the [161]GetConnectionCredentials method in future.
...
Method: org.freedesktop.DBus.AddMatch
   ...
   Adds a match rule to match messages going through the message bus (see [162]the section called “Match Rules”). If the bus does not have enough resources the org.freedesktop.DBus.Error.OOM error is returned.
...
Method: org.freedesktop.DBus.RemoveMatch
   ...
   Removes the first rule that matches (see [163]the section called “Match Rules”). If the rule is not found the org.freedesktop.DBus.Error.MatchRuleNotFound error is returned.
...
Method: org.freedesktop.DBus.GetId
   ...
   Gets the unique ID of the bus. The unique ID here is shared among all addresses the bus daemon is listening on (TCP, UNIX domain socket, etc.) and its format is described in [164]the section called “UUIDs”. Each address the bus is listening on also has its own unique ID, as described in [165]the section called “Server Addresses”. The per-bus and per-address IDs are not related. There is also a per-machine ID, described in [166]the section called “org.freedesktop.DBus.Peer” and returned by org.freedesktop.DBus.Peer.GetMachineId(). For a desktop session bus, the bus ID can be used as a way to uniquely identify a user's session.
...
Property: org.freedesktop.DBus.Features
   ...
   This property lists abstract “features” provided by the message bus, and can be used by clients to detect the capabilities of the message bus with which they are communicating. This property was added in version 1.11.x of the reference implementation of the message bus. Like all properties, it is accessed via method calls to the standard [167]Properties interface.
   ...
   ActivatableServicesChanged
          This message bus emits the ActivatableServicesChanged signal whenever its list of activatable services might have changed (for details see [168]the section called “Signal: org.freedesktop.DBus.ActivatableServicesChanged”).

   AppArmor
          This message bus filters messages via the [169]AppArmor security framework. This feature should only be advertised if AppArmor mediation is enabled and active at runtime; merely compiling in support for AppArmor should not result in this feature being advertised on message bus instances where it is disabled by message bus or operating system configuration.
   ...
   SELinux
          This message bus filters messages via the [170]SELinux security framework. Similar to AppArmor, this feature should only be advertised if SELinux mediation is enabled and active at runtime (if SELinux is placed in permissive mode, that is still considered to be active).

   SystemdActivation
          When asked to activate a service that has the SystemdService field in its .service file, this message bus will carry out systemd activation (for details see [171]the section called “systemd Activation”).
...
Property: org.freedesktop.DBus.Interfaces
   ...
   This property lists interfaces provided by the /org/freedesktop/DBus object, and can be used by clients to detect the capabilities of the message bus with which they are communicating. Unlike the standard Introspectable interface, querying this property does not require parsing XML. This property was added in version 1.11.x of the reference implementation of the message bus. Like all properties, it is accessed via method calls to the standard [172]Properties interface.
...
Message Bus Introspection
   ...
   The message bus should implement the standard [173]Introspectable interface.

Message Bus as a Peer

   The message bus should implement the standard [174]Peer interface. Support for the Peer interface was added in version 1.11.x of the reference implementation of the message bus.

Message Bus Properties

   The message bus should implement the standard [175]Properties interface on the object path /org/freedesktop/DBus. The specific properties to be provided are documented as part of their respective interfaces. Support for the Properties interface was added in version 1.11.x of the reference implementation of the message bus.
...
Method: org.freedesktop.DBus.Monitoring.BecomeMonitor
   ...
   Converts the connection into a monitor connection which can be used as a debugging/monitoring tool. Only a user who is privileged on this bus (by some implementation-specific definition) may create monitor connections[176]^[4].
...
Glossary
   ...
   Bus Name
          The message bus maintains an association between names and connections. (Normally, there's one connection per application.) A bus name is simply an identifier used to locate connections. For example, the hypothetical com.yoyodyne.Screensaver name might be used to send a message to a screensaver from Yoyodyne Corporation. An application is said to own a name if the message bus has associated the application's connection with the name. Names may also have queued owners (see [177]Queued Name Owner). The bus assigns a unique name to each connection, see [178]Unique Connection Name. Other names can be thought of as "well-known names" and are used to find applications that offer specific functionality.

          See [179]the section called “Bus names” for details of the syntax and naming conventions for bus names.
   ...
   Name
          See [180]Bus Name. "Name" may also be used to refer to some of the other names in D-Bus, such as interface names.

   Namespace
          Used to prevent collisions when defining new interfaces, bus names etc. The convention used is the same one Java uses for defining classes: a reversed domain name. See [181]the section called “Bus names”, [182]the section called “Interface names”, [183]the section called “Error names”, [184]the section called “Valid Object Paths”.
   ...
   Service Description Files
          ".service files" tell the bus about service applications that can be launched (see [185]Service). Most importantly they provide a mapping from bus names to services that will request those names when they start up.
   ...
   [186]^[1] Lockfiles are used instead of real file locking fcntl() because real locking implementations are still flaky on network filesystems.

   [187]^[2] This idiosyncratic quoting style is based on the rules for escaping items to appear inside single-quoted strings in POSIX /bin/sh, but please note that backslashes that are not inside single quotes have different behaviour. This syntax does not offer any way to represent an apostrophe inside single quotes (it is necessary to leave the single-quoted section, backslash-escape the apostrophe and re-enter single quotes), or to represent a comma outside single quotes (it is necessary to wrap it in a single-quoted section).

   [188]^[3] The system bus is security-sensitive and is typically executed by an init system with a clean environment. Its launch helper process is particularly security-sensitive, and specifically clears its own environment.

   [189]^[4] In the reference implementation, the default configuration is that each user (identified by numeric user ID) may monitor their own session bus, and the root user (user ID zero) may monitor the system bus.

References

   Visible links:
   1. mailto:hp@pobox.com
   2. mailto:andersca@codefactory.se
   3. mailto:alexl@redhat.com
   4. mailto:sven@imendio.com
   5. mailto:smcv@collabora.com
   6. mailto:zeuthen@gmail.com
   7. http://cgit.freedesktop.org/dbus/dbus/log/doc/dbus-specification.xml
   8. https://dbus.freedesktop.org/doc/dbus-specification.html#introduction
   9. https://dbus.freedesktop.org/doc/dbus-specification.html#stability
  10. https://dbus.freedesktop.org/doc/dbus-specification.html#type-system
  11. https://dbus.freedesktop.org/doc/dbus-specification.html#basic-types
  12. https://dbus.freedesktop.org/doc/dbus-specification.html#container-types
  13. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.3.8
  14. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-marshaling
  15. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.4.3
  16. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.4.4
  17. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.4.5
  18. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.4.6
  19. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol
  20. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-messages
  21. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names
  22. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-types
  23. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-handling-invalid
  24. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-protocol
  25. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-protocol-overview
  26. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-nul-byte
  27. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-auth
  28. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-cancel
  29. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-data
  30. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-begin
  31. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-rejected
  32. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-ok
  33. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-error
  34. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-negotiate-unix-fd
  35. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-agree-unix-fd
  36. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-command-future
  37. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-examples
  38. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-states
  39. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-mechanisms
  40. https://dbus.freedesktop.org/doc/dbus-specification.html#addresses
  41. https://dbus.freedesktop.org/doc/dbus-specification.html#transports
  42. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-unix-domain-sockets
  43. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-launchd
  44. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-systemd
  45. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-tcp-sockets
  46. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-nonce-tcp-sockets
  47. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-exec
  48. https://dbus.freedesktop.org/doc/dbus-specification.html#meta-transports
  49. https://dbus.freedesktop.org/doc/dbus-specification.html#meta-transports-autolaunch
  50. https://dbus.freedesktop.org/doc/dbus-specification.html#uuids
  51. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces
  52. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-peer
  53. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-introspectable
  54. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-properties
  55. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-objectmanager
  56. https://dbus.freedesktop.org/doc/dbus-specification.html#introspection-format
  57. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus
  58. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-overview
  59. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-names
  60. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-routing
  61. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-starting-services
  62. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-types
  63. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-messages
  64. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.13.8
  65. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.13.9
  66. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-properties
  67. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-monitoring-interface
  68. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-debug-stats-interface
  69. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-verbose-interface
  70. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.14
  71. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol
  72. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus
  73. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-marshaling-object-path
  74. https://dbus.freedesktop.org/doc/dbus-specification.html#term-single-complete-type
  75. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-interface
  76. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-bus
  77. https://bugs.freedesktop.org/show_bug.cgi?id=27857
  78. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-messages
  79. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-marshaling-signature
  80. https://dbus.freedesktop.org/doc/dbus-specification.html#type-system
  81. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-properties-features
  82. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus
  83. https://datatracker.ietf.org/doc/html/rfc5890
  84. http://docs.flatpak.org/en/latest/introduction.html#identifiers
  85. https://specifications.freedesktop.org/desktop-entry-spec/desktop-entry-spec-latest.html#dbus
  86. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-interface
  87. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-interface
  88. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-starting-services
  89. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-protocol
  90. https://dbus.freedesktop.org/doc/dbus-specification.html#addresses
  91. https://dbus.freedesktop.org/doc/dbus-specification.html#addresses
  92. https://tools.ietf.org/html/rfc4422#appendix-A
  93. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-unix-domain-sockets-addresses
  94. https://dbus.freedesktop.org/doc/dbus-specification.html#ftn.id-1.6.20.4.9.1.1.1.2
  95. https://tools.ietf.org/html/rfc4505
  96. https://dbus.freedesktop.org/doc/dbus-specification.html#uuids
  97. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-systemd
  98. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-launchd
  99. https://man7.org/linux/man-pages/man7/network_namespaces.7.html
 100. https://man7.org/linux/man-pages/man2/chroot.2.html
 101. https://man7.org/linux/man-pages/man7/mount_namespaces.7.html
 102. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-mechanisms-external
 103. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-mechanisms-sha
 104. https://lists.freedesktop.org/archives/dbus/2018-April/017447.html
 105. https://dbus.freedesktop.org/doc/dbus-specification.html#auth-mechanisms-sha
 106. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-tcp-sockets
 107. https://dbus.freedesktop.org/doc/dbus-specification.html#transports-tcp-sockets
 108. https://dbus.freedesktop.org/doc/dbus-specification.html#addresses
 109. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-peer
 110. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-types-notation
 111. https://www.freedesktop.org/software/systemd/man/machine-id.html
 112. https://docs.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-getcurrenthwprofilea
 113. https://dbus.freedesktop.org/doc/dbus-specification.html#uuids
 114. https://dbus.freedesktop.org/doc/dbus-specification.html#introspection-format
 115. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-member
 116. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-introspectable
 117. https://dbus.freedesktop.org/doc/dbus-specification.html#introspection-format
 118. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-properties
 119. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-properties
 120. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-introspectable
 121. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-properties
 122. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-routing
 123. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-bus
 124. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-header-fields
 125. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-add-match
 126. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-become-monitor
 127. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-routing-match-rules
 128. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-routing-eavesdropping
 129. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-become-monitor
 130. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-add-match
 131. https://dbus.freedesktop.org/doc/dbus-specification.html#ftn.id-1.13.4.9.6.3
 132. https://dbus.freedesktop.org/doc/dbus-specification.html#term-bus-name
 133. https://dbus.freedesktop.org/doc/dbus-specification.html#term-unique-name
 134. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-interface
 135. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-marshaling-object-path
 136. https://dbus.freedesktop.org/doc/dbus-specification.html#term-unique-name
 137. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-name-owner-changed
 138. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-routing-eavesdropping
 139. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-become-monitor
 140. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-start-service-by-name
 141. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-types
 142. http://standards.freedesktop.org/desktop-entry-spec/desktop-entry-spec-latest.html
 143. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-types
 144. https://www.freedesktop.org/wiki/Software/systemd/
 145. http://wiki.apparmor.net/index.php/Documentation
 146. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-start-service-by-name
 147. http://standards.freedesktop.org/basedir-spec/basedir-spec-latest.html
 148. https://dbus.freedesktop.org/doc/dbus-specification.html#ftn.id-1.13.6.4.6.8
 149. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-name-owner-changed
 150. https://dbus.freedesktop.org/doc/dbus-specification.html#term-queued-owner
 151. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-list-activatable-names
 152. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-properties-features
 153. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-starting-services
 154. https://en.wikipedia.org/wiki/Time_of_check_to_time_of_use
 155. https://dbus.freedesktop.org/doc/dbus-specification.html#ftn.id-1.13.7.20.4.1.1.2.6.3.1.1
 156. https://dbus.freedesktop.org/doc/dbus-specification.html#ftn.id-1.13.7.20.4.1.1.2.6.3.2.2
 157. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.13.7.20.4.1.1.2.6.3.1.1
 158. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.13.7.20.4.1.1.2.6.3.2.2
 159. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-get-connection-unix-user
 160. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-get-connection-credentials
 161. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-get-connection-credentials
 162. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-routing-match-rules
 163. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-routing-match-rules
 164. https://dbus.freedesktop.org/doc/dbus-specification.html#uuids
 165. https://dbus.freedesktop.org/doc/dbus-specification.html#addresses
 166. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-peer
 167. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-properties
 168. https://dbus.freedesktop.org/doc/dbus-specification.html#bus-messages-activatable-services-changed
 169. http://wiki.apparmor.net/
 170. https://selinuxproject.org/
 171. https://dbus.freedesktop.org/doc/dbus-specification.html#message-bus-starting-services-systemd
 172. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-properties
 173. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-introspectable
 174. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-peer
 175. https://dbus.freedesktop.org/doc/dbus-specification.html#standard-interfaces-properties
 176. https://dbus.freedesktop.org/doc/dbus-specification.html#ftn.id-1.13.11.3.3.2
 177. https://dbus.freedesktop.org/doc/dbus-specification.html#term-queued-owner
 178. https://dbus.freedesktop.org/doc/dbus-specification.html#term-unique-name
 179. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-bus
 180. https://dbus.freedesktop.org/doc/dbus-specification.html#term-bus-name
 181. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-bus
 182. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-interface
 183. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-names-error
 184. https://dbus.freedesktop.org/doc/dbus-specification.html#message-protocol-marshaling-object-path
 185. https://dbus.freedesktop.org/doc/dbus-specification.html#term-service
 186. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.6.20.4.9.1.1.1.2
 187. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.13.4.9.6.3
 188. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.13.6.4.6.8
 189. https://dbus.freedesktop.org/doc/dbus-specification.html#id-1.13.11.3.3.2

   Hidden links:
 191. https://dbus.freedesktop.org/doc/dbus-specification.html#term-single-complete-type
